...
public class AdminStayRoomWriteAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		while(stayRoom_photo.hasMoreElements()){
			String stayRoom_files = (String)stayRoom_photo.nextElement(); // 객체 반환 / String으로 형변환
			File stayRoomFile = multi.getFile(stayRoom_files); // 업로드된 파일의 파일 객체 반환
			
			if (stayRoomFile != null){
				// fileExt : 추출된 파일명
				String fileExt = stayRoomFile.getName().substring(stayRoomFile.getName().lastIndexOf(".") + 1); // 확장자 자르기
				System.out.println(stayRoomFile.getName().lastIndexOf("."));
