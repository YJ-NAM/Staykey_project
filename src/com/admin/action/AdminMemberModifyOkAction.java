...
public class AdminMemberModifyOkAction implements Action {
    ...
    public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 비밀번호 변경 값
		String member_pw_chg = multi.getParameter("member_pw_chg");
		
		// 비밀번호 변경 값이 없으면 기존에 히든으로 넘겨줬던 비밀번호 값을 받아준다.
		if(member_pw_chg == null) {
			member_pw_chg = multi.getParameter("member_pw");
		}
        ...
        dto.setMember_pw(member_pw_chg);
        dto.setMember_name(member_name);
        dto.setMember_email(member_email);
        dto.setMember_phone(member_phone);
        
        System.out.println(">" + member_pw_chg);
